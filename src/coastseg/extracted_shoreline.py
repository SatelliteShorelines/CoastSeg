...
class Extracted_Shoreline:
    ...
    def __init__(
        ...
        if not isinstance(shoreline,gpd.GeoDataFrame):
            raise ValueError(f"shoreline must be valid geodataframe. not {type(shoreline)}")
        if shoreline.empty:
        ...
        self.extracted_shorelines = self.extract_shorelines(shoreline,roi_settings,settings)
    ...
    def extract_shorelines(
        ...
        reference_shoreline = get_reference_shoreline(shoreline_gdf,output_crs,map_crs)
    ...
    def create_shoreline_settings(self, settings:dict,roi_settings:dict,reference_shoreline:dict)-> None:
    ...
    def create_geodataframe(
        ...
        if output_crs is not None:
            extract_shoreline_gdf.to_crs(output_crs)
        return extract_shoreline_gdf

    def save_to_file(self,sitename:str,filepath:str):
        ...
        self.gdf.to_file(
            os.path.join(filepath, sitename, f"extracted_shorelines.geojson"),
            driver="GeoJSON",
            encoding="utf-8",
        )
...
def get_reference_shoreline(shoreline_gdf: gpd.geodataframe,output_crs:str,input_crs=4326)->np.ndarray:
