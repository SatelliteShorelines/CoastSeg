...
class CoastSeg_Map:
    ...
    def save_config(self,is_downloaded:bool = True)->None:
        ...
        if self.selected_ROI_layer is None:
            logger.error("No ROIs were selected. Cannot save ROIs to config until ROIs are selected.")
            raise Exception("No ROIs were selected. Cannot save ROIs to config until ROIs are selected.")
        ...
        if self.rois.inputs_dict is not None and is_downloaded:
            logger.info(f"save_config :: inputs from inputs_dict: {self.rois.inputs_dict}")
    ...
    def extract_all_shorelines(self)->None:
        ...
        for id in tqdm(roi_ids, desc="Extracting Shorelines"):
            try:
                ...
                extracted_shoreline=self.extract_shorelines_from_roi(selected_rois_gdf,self.shoreline.gdf,inputs,settings,id=id)
                logger.info(f"extract_all_shorelines : ROI: {id} extracted_shoreline: {extracted_shoreline}")
    ...
    def extract_shorelines_from_roi(self, rois_gdf:gpd.GeoDataFrame, shorelines_gdf: gpd.geodataframe, inputs:dict, settings: dict, id:int=None)->dict:
        ...
        if extract_shoreline_gdf is None:
            ...
            extract_shoreline_gdf.to_file(os.path.join(filepath, sitename, f'shorelines_{id}.geojson'),
    ...
    def compute_transects_from_roi(self,roi_id:int,inProj:Proj, outProj: Proj, settings:dict)-> dict:
        ...
        try:
            ...
            if not common.is_shoreline_present(self.rois.extracted_shorelines,str(roi_id)):
                raise exceptions.No_Extracted_Shoreline(str(roi_id),"No extracted shoreline at this roi")
            else:
    ...
    def compute_transects(self) -> dict:
        ...
        if len(roi_ids) == 0:
            logger.error(f"self.selected_set: {self.selected_set} was not a subset of extracted_shoreline_ids: {set(extracted_shoreline_ids)}")
            raise Exception(f"You must select an ROI and extract it shorelines before you can compute transects")
