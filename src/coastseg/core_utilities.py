...
def get_base_dir(repo_name: str = "CoastSeg") -> pathlib.Path:
    ...
    def resolve_repo_path(
        cwd: pathlib.Path, proj_name: str, max_depth: int = 100
    ) -> pathlib.Path:
        root = (
            cwd.anchor if cwd.anchor else cwd.root
        )  # Handle root differently for Windows
        # print(f"root: {root}")
        proj_dir = cwd
        depth = 0
        ...
        while proj_dir.name != proj_name:
            if depth > max_depth:
                raise ValueError(
                    f"Reached maximum depth - cannot resolve project path. Could not find {proj_name} in {cwd}."
                )
            proj_dir = proj_dir.parent
            depth += 1
            if str(proj_dir) == root:
                raise ValueError(
                    f"Reached root depth - cannot resolve project path. Could not find {proj_name} in {cwd}."
                )
