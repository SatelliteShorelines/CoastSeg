...
def test_count_images_in_ee_collection(mock_ee, polygon, start_date, end_date):
    mock_ee.ImageCollection.return_value.filterBounds.return_value.filterDate.return_value.filterMetadata.return_value.filter.return_value.size.return_value.getInfo.return_value = 10
    ...
    assert result == {"L5": 20, "L7": 20, "L8": 20, "L9": 10, "S2": 10}
...
def test_count_images_in_ee_collection_uninitialized_ee(mock_ee, polygon, start_date, end_date):
...
def test_count_images_in_ee_collection_custom_months(mock_ee, polygon, start_date, end_date):
    mock_ee.ImageCollection.return_value.filterBounds.return_value.filterDate.return_value.filterMetadata.return_value.filter.return_value.size.return_value.getInfo.return_value = 5

    result = downloads.count_images_in_ee_collection(polygon, start_date, end_date, months_list=[12, 1])
    ...
    assert result == {"L5": 10, "L7": 10, "L8": 10, "L9": 5, "S2": 5}

def test_get_collection_by_tier(mock_ee, polygon, start_date, end_date):
    mock_ee.ImageCollection.return_value.filterBounds.return_value.filterDate.return_value.filterMetadata.return_value.filter.return_value = MagicMock()
...
def test_get_collection_by_tier_invalid_satellite(mock_ee, polygon, start_date, end_date):
...
def test_get_collection_by_tier_custom_months(mock_ee, polygon, start_date, end_date):
    ...
    result = downloads.get_collection_by_tier(polygon, start_date, end_date, "L8", 1, months_list=[12, 1])
