...
def test_create_geodataframe(valid_bbox_gdf:gpd.GeoDataFrame,valid_shoreline_gdf:gpd.GeoDataFrame, valid_ROI:roi.ROI):
    ...
    actual_gdf = valid_ROI.create_geodataframe(bbox = valid_bbox_gdf,
    ...
    assert (valid_ROI.gdf.dtypes['id'] == 'int32') or (valid_ROI.gdf.dtypes['id'] == 'int64')
...
def test_fishnet_intersection(valid_bbox_gdf:gpd.GeoDataFrame,valid_shoreline_gdf:gpd.GeoDataFrame, valid_ROI:roi.ROI):
...
def test_get_fishnet(valid_bbox_gdf:gpd.GeoDataFrame,valid_shoreline_gdf:gpd.GeoDataFrame, valid_ROI:roi.ROI):
    # tests if a valid fishnet geodataframe intersects with given shoreline geodataframe
    square_size=1000
    output_espg="epsg:4326"
    # check if fishnet intersects the shoreline
    fishnet_gdf = valid_ROI.get_fishnet_gdf(bbox_gdf = valid_bbox_gdf,
...
def test_create_fishnet(valid_bbox_gdf:gpd.GeoDataFrame, valid_ROI:roi.ROI):
    ...
    actual_fishnet =  valid_ROI.create_fishnet(valid_bbox_gdf,
...
def test_create_rois(valid_ROI:roi.ROI,valid_bbox_gdf:gpd.GeoDataFrame):
    ...
    input_espg = "epsg:4326"
    output_espg ="epsg:4326"
    actual_roi_gdf=valid_ROI.create_rois(bbox=valid_bbox_gdf,
                                          input_espg =input_espg,
                                          output_espg=output_espg,
                                          square_size=square_size)
...
def test_valid_roi_gdf(transect_compatible_rois_gdf:gpd.GeoDataFrame):
    ...
    assert (actual_roi.gdf.dtypes['id'] == 'int32') or (actual_roi.gdf.dtypes['id'] == 'int64')
    assert actual_roi.filename == "rois.geojson"
    assert hasattr(actual_roi, "extracted_shorelines")
    assert hasattr(actual_roi, "cross_distance_transects")
    assert hasattr(actual_roi, "inputs_dict")
