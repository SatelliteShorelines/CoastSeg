...
from CoastSeg import SDS_tools, SDS_download, SDS_preprocess, SDS_shoreline, SDS_transects


def download_imagery_with_metadata(inputs_file:str,pre_process_settings:dict):
    """
    ...
    """
    assert os.path.exists(inputs_file),"Path to inputs_file {inputs_file} did not exist"
    ...
    for inputs in tqdm_notebook(inputs_list,desc="Downloading ROIs with metadata"):
        print("\ninputs: ",inputs,"\n")
...
def download_imagery(selected_roi_geojson:dict,pre_process_settings:dict,dates:list,sat_list:list,inputs_filename="inputs.json") -> None:
    """
...
#     1. Check imagery available and check for ee credentials
    try:
        inputs_list=check_images_available_selected_ROI(selected_roi_geojson,dates,sat_list)
        print(inputs_list)
...
def save_roi(geojson_file:str, selected_roi_file: str, selected_roi_set: set)-> dict:
    """
    ...
    selected_ROI_geojson= get_selected_roi_geojson(selected_roi_set, geojson_data)
    #3. Save the rois' geojson data to a file named selected_roi_file
    make_overlapping_roi.ROI.write_to_geojson_file(selected_roi_file,selected_ROI_geojson,perserve_id=True)
...
def check_images_available_selected_ROI(selected_roi_geojson:dict,dates:list,sat_list:list)->list:
    """"
