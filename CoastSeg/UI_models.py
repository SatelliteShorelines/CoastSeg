...
class UI_Models:
    ...
    def __init__(self):
        ...
        self.RGB_models=['landsat_6229071', 'landsat_6230083','SWED-RGB_6824384','coast-train-RGB_6950479', 'S2-water-SWED_6950474']
    ...
    def create_dashboard(self):
        model_choices_box=HBox([self.model_input_dropdown,self.model_dropdown, self.model_implementation])
        checkboxes = HBox([self.GPU_checkbox,self.CRF_checkbox])
        instr_vbox=VBox([self.instr_header, self.line_widget, self.instr_use_data, self.instr_select_images,
                         self.instr_run_model])
        display(checkboxes, UI_Models.GPU_view,model_choices_box,instr_vbox,self.use_data_button,
                self.use_select_images_button,self.line_widget,
                UI_Models.model_view,self.run_model_button,UI_Models.run_model_view,self.open_results_button)
    ...
    def _create_buttons(self):
        ...
        self.use_select_images_button = Button(description="Select Your Images",style=load_style)
        self.use_select_images_button.on_click(self.use_select_images_button_clicked)
        self.open_results_button = Button(description="Open Model Results",style=load_style)
    ...
    def _create_HTML_widgets(self):
        ...
        widgets created: instr_create_ro, instr_save_roi, instr_load_btns
         instr_download_roi
        """
        self.line_widget=HTML(value="____________________________________________________")
    ...
    def run_model_button_clicked(self, button):
        ...
        if self.model_dict["sample_direc"] is None:
            with Tkinter_Window_Creator():
                messagebox.showinfo("","You must click 'Use Data' or 'Select Images' First")
            return
        else:
            ...
            zoo_model.compute_segmentation(self.model_dict["sample_direc"], model_list, metadatadict,self.model_dict['use_CRF'])
        ...
        with Tkinter_Window_Creator():
            if self.model_dict["sample_direc"] is None:
                ...
                if not os.path.exists(model_results_path):
                    messagebox.showerror("File Not Found","The directory for the model outputs could not be found")
    ...
    def use_select_images_button_clicked(self, button):
        ...
        with Tkinter_Window_Creator() as tk_root:
            ...
            if os.path.exists(data_path):
                ...
                data_path = os.getcwd()
            tk_root.filename =  filedialog.askdirectory(initialdir = data_path,title = "Select directory of images",)
            ...
            if tk_root.filename:
                ...
                jpgs = glob.glob1(sample_direc + os.sep, "*jpg")
                if jpgs == []:
                    messagebox.showerror("No JPGs Found", "\nERROR!\nThe directory contains no jpgs! Please select a directory with jpgs.")
                elif jpgs != []:
                    self.model_dict["sample_direc"]=sample_direc
            else:
                messagebox.showerror("Invalid directory", "You must select a valid directory first!")