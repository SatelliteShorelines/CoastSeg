...
def get_ROIs(coastline : dict,roi_filename:str,csv_filename:str):
    ...
    # master_overlap_df: geopandas dataframe containing all the overlap data for the rois
    start_id=0
    ...
    for line in tqdm_notebook(lines_list,desc="Calculating Overlap"):
        ...
        end_id=write_to_geojson_file(TEMP_FILENAME,geojson_polygons,perserve_id=False,start_id=start_id)
        ...
        # THIS MAY NOT BE CORRECT
            ...
            start_id=end_id
        master_overlap_df=master_overlap_df.append(overlap_df, ignore_index=False)
    ...
    if(length_vector_bbox_features != 1):
        for i in range(0,length_vector_bbox_features):
            if  vector_in_bbox_geojson['features'][i]['geometry']['type'] == 'MultiLineString':
                for y in range(len(vector_in_bbox_geojson['features'][i]['geometry']['coordinates'])):
                    line=LineString(vector_in_bbox_geojson['features'][i]['geometry']['coordinates'][y])
                    lines_list.append(line)
            elif  vector_in_bbox_geojson['features'][i]['geometry']['type'] == 'LineString':
                line=LineString(vector_in_bbox_geojson['features'][i]['geometry']['coordinates'])
                lines_list.append(line)
    else:
        for i in range(0,len(vector_in_bbox_geojson['features'])):
            if  vector_in_bbox_geojson['features'][0]['geometry']['type'] == 'MultiLineString':
                for y in range(len(vector_in_bbox_geojson['features'][0]['geometry']['coordinates'])):
                    line=LineString(vector_in_bbox_geojson['features'][0]['geometry']['coordinates'][y])
                    lines_list.append(line)
            elif  vector_in_bbox_geojson['features'][i]['geometry']['type'] == 'LineString':
                line=LineString(vector_in_bbox_geojson['features'][i]['geometry']['coordinates'])
                lines_list.append(line)
...
# Keep looping while not all the rois overlap and the average overlap is more than 80%
    while do_all_ROI_overlap== False and  is_overlap_excessive== True:
        ...
        tuples_list=convert_multipoints_to_tuples(multipoint_list)
        geojson_polygons=create_reactangles(tuples_list)
        end_id=write_to_geojson_file(filename,geojson_polygons,perserve_id=False,start_id=start_id)
