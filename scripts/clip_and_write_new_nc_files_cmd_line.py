...
def clip_specific_region(files,geometries,region_number=10):
    ...
    for counter, k in enumerate(files):  
        ...
        output.to_netcdf(path=k.replace('.nc',f'_clipped_region{region_number}.nc'), mode='w', format=None, group=None, engine=None, encoding=None, unlimited_dims=None, compute=True, invalid_netcdf=False)
...
# Add the arguments
parser.add_argument('--tide_model','-T',default='',
                    type=str,
                    help='Path to tide model directory')

parser.add_argument('--regions_file','-R',default='',
                    type=str,
...
load_tide_nc_files = [f for f in glob(os.path.join(load_tide_dir,'*.nc')) if 'clipped' not in f]
ocean_tide_nc_files = [f for f in glob(os.path.join(ocean_tide_dir,'*.nc')) if 'clipped' not in f]
